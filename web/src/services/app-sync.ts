...
const imageLogStore = localforage.createInstance({ name: "visora", storeName: "image_generation_logs" });
const videoLogStore = localforage.createInstance({ name: "visora", storeName: "video_generation_logs" });
...
async function syncDomain<T>(config: WebdavSyncConfig, onProgress: AppSyncProgress | undefined, options: SyncDomainOptions<T>): Promise<SyncDomainResult<T>> {
    try {
        ...
        const manifest: DomainManifest<T> = { app: "visora", version: 1, domain: options.key, exportedAt: new Date().toISOString(), data: mergedData, files: uploaded.files };
...
async function readDomainManifest<T>(config: WebdavSyncConfig, domain: DomainKey, emptyData: T): Promise<DomainManifest<T> | null> {
    ...
    if (data.app !== "visora" || data.domain !== domain) throw new Error(i18n.t("config.webdav.errors.invalidManifest", { domain }));
    return {
        app: "visora",
        version: 1,
        domain,
