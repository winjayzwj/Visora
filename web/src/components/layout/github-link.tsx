...
export function GitHubLink({ className, style }: GitHubLinkProps) {
    ...
    return (
        <a
            className={cn("inline-flex size-9 shrink-0 items-center justify-center rounded-full text-[var(--studio-muted)] transition hover:bg-[var(--surface-hover)] hover:text-[var(--studio-text)]", className)}
            style={style}
            href={REPOSITORY_URL}
            target="_blank"
            rel="noreferrer"
