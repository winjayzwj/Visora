...
export async function startMcpServer() {
    ...
    const server = new McpServer({ name: "visora", version: VERSION }, { instructions: AGENT_PROMPT });
    toolNames.forEach((name) => registerCanvasTool(server, config, name));
    await server.connect(new StdioServerTransport());
}

/** 向 MCP Server 注册单个 Visora AI Agent 工具。 */
...
/** 将 MCP 工具调用转发到本地 Visora AI Agent HTTP 服务。 */
async function postCanvasAgentTool(config: CanvasAgentConfig, name: ToolName, input: unknown) {
    const res = await fetch(`${config.url}/api/tools`, { method: "POST", headers: { "content-type": "application/json", "x-visora-agent-token": config.token }, body: JSON.stringify({ name, input }) });
