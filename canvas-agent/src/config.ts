...
export const CONFIG_DIR = path.join(os.homedir(), ".visora");
export const CONFIG_FILE = path.join(CONFIG_DIR, "visora-agent.json");
...
/** 读取本地 Visora AI Agent 配置，不存在时生成默认配置。 */
...
function initializeWorkspace(workspacePath: string) {
    ...
    if (!current || current.startsWith("# Visora AI Agent")) fs.writeFileSync(instructionsFile, AGENT_PROMPT);
