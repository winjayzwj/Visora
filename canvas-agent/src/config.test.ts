...
function makeTempBase(): string {
    return fs.mkdtempSync(path.join(os.tmpdir(), "visora-agent-config-test-"));
}
...
test("writeConfigFile creates directory 0700 and file 0600", posix, () => {
    ...
    const file = path.join(dir, "visora-agent.json");
    try {
...
test("writeConfigFile persists the config content", () => {
    const base = makeTempBase();
    const file = path.join(base, "visora-agent.json");
    try {
