...
export function checkVersions() {
    ...
    logger.info("Visora AI Agent version", { version: VERSION });
    ...
    if (!localCodexVersion) {
        ...
        logger.warn(`Bundled Codex ${CODEX_VERSION} does not match local Codex ${localCodexVersion}. Keep both current with: npm install -g @openai/codex@latest; rebuild the local Agent after updating its bundled Codex dependency`);
    }
...
async function checkLatestVersions(localCodexVersion: string) {
    try {
        ...
        if (isOlder(CODEX_VERSION, latestCodex)) logger.warn(`Update available: bundled Codex ${CODEX_VERSION} -> ${latestCodex}. Update the bundled Codex dependency in canvas-agent/package.json, then reinstall dependencies and rebuild locally`);
