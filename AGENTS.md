...
- 按改动影响执行最小充分验证：纯文案或低风险配置做针对性检查；影响行为、构建、数据格式或发布时运行相关语法、测试或构建。无法执行时明确说明缺口。
- 不要改无关文件，不要顺手重构。
...
- 用户端请求统一放在 `web/src/services/api/`，管理端请求放在 `admin/src/services/api/`；新增平台账号能力通过同源 `/api/` 访问本项目 `server/` 的 Go 服务。既有本地创作的第三方直连暂时保留，后续按实施阶段接入平台，不能把公司 AI Host 当成本项目后端。
...
- 用户端主题继续使用 `web/src/lib/app-theme.ts` 与 `AppProviders`；独立 `admin/` 的主题、背景、表格配色在其全局主题入口或必要的全局 CSS 作用域配置，不让 Admin 依赖用户端业务组件。页面私有组件不要自己写 `dark ? ...` 主题分支。
...
- 页面文案保持中文。
- 新增或修改面向用户的文案必须同时更新 `web/src/i18n/locales/zh-CN.ts` 与 `web/src/i18n/locales/en-US.ts`，业务组件不得新增硬编码用户文案。
- 新增或调整用户端 UI 必须使用语义主题 token，并在浅色、深色主题下验证文字、边界与交互态可读；不新增只覆盖单一主题的样式。
...
- HeroUI `Surface` 仅用于独立工作台面板和真实内容卡片；搜索框、筛选栏、工具栏、加载占位和空状态不得机械套成岛屿。避免同一内容区嵌套底色与边框，调整留白时同时检查父级 gap 与子级 margin，防止间距叠加。
- `/assets` 与 `/prompts` 的页面主内容区使用 HeroUI `Card` 作为外层容器，内部瀑布流内容项也使用 `Card`；搜索和筛选控件保持为卡片内的直接控件。
- 用户端弹窗与抽屉右上关闭控件统一使用 HeroUI 的 icon-only tertiary Button（`X` 图标），默认按钮采用 `foreground` 语义色；primary、danger、disabled 等状态保留其语义颜色。

- 用户端允许 Shadcn 与 HeroUI v3 并存，AI 图像与视频工作台优先使用 Shadcn；两套组件共享现有主题语义 token。不要退回 Ant Design、硬编码深浅主题颜色或新增另一套配色。
- 设计优先保证内容可读、留白合理和操作路径清楚。不要把高频创作工具做成营销页式大卡片，也不要为装饰加入无意义动效；浅色与深色主题必须同时成立。
- 工作台和表单类页面默认采用紧凑、分组明确的布局：关联配置（例如画幅和生成张数）在桌面端同排，窄屏才堆叠；常用入口使用短文案并紧邻放置，避免宽大的重复按钮。
- 图像与视频工作台的图片区只为「添加」「我的创作」入口保留虚线边框，外层拖放区域不再增加大边框或常驻底色。画幅比例与生成张数采用等宽布局，切换任意张数时必须保留完整选中边框。
- 内容可能超出视口的桌面侧栏、弹窗和编辑区域必须保留独立纵向滚动，不得以 `overflow: hidden` 裁切关键操作或形成横向空白滚动。
- 图片、灵感和提示词等内容集合优先按真实图片比例使用紧凑瀑布流；点击内容后的详情优先采用左侧图片、右侧正文的双栏布局，分类和标签放在正文下方。
- 用户明确指出视觉目标或交互方案时直接按该方案实现；若布局、视觉方向、文案或行为存在会显著影响结果的歧义，先用一个简洁问题向用户确认，不自行扩展或猜测。
...
- 仅在用户可感知功能或待办状态实际变化时，检查并更新 `docs/content/docs/progress/todo.mdx` 与 `docs/content/docs/progress/pending-test.mdx`；无变化时无需为此产生文档改动。
...
- 发版本时按变更风险运行必要的编译、测试或构建；用户明确要求跳过时，记录未验证项和风险。
...
- 平台一期采用 `web/` 用户端、`admin/` 独立管理端、`server/` Go 单体与 MongoDB；仅实现当前阶段需要的模块，不提前引入微服务、Redis、消息队列或多租户框架。
- 多代理开发先固定接口契约和各自可写目录；保留品牌统一等已有未提交改动，不允许代理互相覆盖文件或擅自 Git 提交。
