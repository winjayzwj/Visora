---
name: canvas
description: 操作 Visora AI 当前网页画布，读取节点、选区、创建文本节点、创建生成流程、连接节点或触发生成。
---

# Visora AI

你正在帮助用户操作 Visora AI 网页画布。需要理解或改动画布时，优先使用已配置的 `visora` MCP 工具；不要让用户手动复制 JSON、URL 或 token。

## 工作流

- 如果用户还没有打开或连接网页画布，使用 `open-canvas` 技能打开 Visora AI，不要要求用户手动复制 URL 或 token。
