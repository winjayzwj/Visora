...
description: Deploy Visora AI with Docker Compose
---

# Docker Deployment

## Use a configured image

```bash
cd /path/to/Visora
export VISORA_IMAGE=visora:latest
docker compose up -d
```

The root Compose file maps the container to `http://localhost:8888`.
...
services:
  app:
    image: ${VISORA_IMAGE:-visora:latest}
    ports:
      - "8888:3000"
    environment:
