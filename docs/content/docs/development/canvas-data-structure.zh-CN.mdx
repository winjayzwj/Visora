...
- 画布项目 JSON：`localForage`，数据库名 `visora`，storeName `app_state`，key 为 `visora:canvas_store`。
- 我的素材 JSON：`localForage`，数据库名 `visora`，storeName `app_state`，key 为 `visora:asset_store`。
- 图片 Blob：单独存到 `localForage` 实例，数据库名 `visora`，storeName `image_files`。
- 视频等媒体 Blob：单独存到 `localForage` 实例，数据库名 `visora`，storeName `media_files`。
