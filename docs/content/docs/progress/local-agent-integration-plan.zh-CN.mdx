...
- 后续是否发布 npm 包另行评估；当前优先支持从本地源码启动，避免提前绑定发布地址。
...
网页侧边栏助手另走 Canvas Agent 的对话通道。Canvas Agent 使用官方 `@openai/codex` CLI 的 `codex app-server --stdio` 启动并续用同一个 thread，启动时会注入 `visora` MCP 配置并把 `default_tools_approval_mode` 设为 `approve`，避免 Codex 自己的 MCP 审批卡住侧边栏；真正修改画布前由网页侧边栏做二次确认。
...
Canvas Agent 调用 Claude Code 时默认允许 `mcp__visora__*`，避免 print 模式被工具审批卡住；真正修改画布仍由网页侧边栏确认。
...
1. 前端新增本地 Agent 侧边栏。
2. Canvas Agent 通过官方 `@openai/codex` CLI 的 `codex app-server --stdio` 接入 Codex，复用同一个 thread，注入 `visora` MCP，并展示结构化事件流。
...
- 用户运行本地 `canvas-agent` 后，Visora 页面能显示已连接。
